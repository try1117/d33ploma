...
namespace graph_constraint_solver {
    ...
    public:
        ...
        GraphPtr generate_tree_fixed_leaves_number(int order, int leaves_number, double merge_probability = 0.3);
    ...
    private:
        ...
        GraphPtr generate_strongly_connected_component(std::pair<int, int> order_bounds, std::pair<long long, long long> size_bounds);
        ...
        GraphComponentsPtr generate_two_connected_graph(Graph::Type graph_type, int order, int size,
                int components_number, std::pair<int, int> components_order_bounds);
        GraphPtr generate_two_connected_component(Graph::Type graph_type, int order, int size,
                int min_loop_size, double loop_ear_probability = 0.0);

        GraphPtr generate_two_edge_connected_component(Graph::Type graph_type, int order,
                std::pair<long long, long long> size_bounds, int cut_points);

        GraphPtr generate_connected_component(Graph::Type graph_type, Utils::IntPair order_bounds,
